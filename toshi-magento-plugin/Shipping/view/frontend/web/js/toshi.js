...
function launchToshi() {
    ...
    window.showErrorMessage = () => {
        ...
        checkoutConfig.quoteItemData.forEach((item, index) => {
            ...
            availabilityType = checkoutConfig.toshiData.products[index].availabilityType;
            availabilityDate = checkoutConfig.toshiData.products[index].availabilityDate;

            products.push(createProduct(item.name, item.sku, item.qty, item.thumbnail, item.base_price_incl_tax, getAttribute(item, sizeAttribute), getAttribute(item, colorAttribute), availableSizes, availabilityType, availabilityDate));
        });
